...
class ViewInjectUtils {
    ...
    private static void injectEvents(Activity activity)
    {
        ...
        //遍历所有的方法
        Log.d("ggg", "methods---" + methods);
        for (Method method : methods)
        {
            Log.d("ggg", "method---" + method);
            Annotation[] annotations = method.getAnnotations();
            Log.d("ggg", "annotations---" + annotations);
            //拿到方法上的所有的注解
            for (Annotation annotation : annotations)
            {
                Class<? extends Annotation> annotationType = annotation
                        .annotationType();
                Log.d("ggg", "annotationType---" + annotationType);
                //拿到注解上的注解
                EventBase eventBaseAnnotation = annotationType
                        .getAnnotation(EventBase.class);
                Log.d("ggg", "eventBaseAnnotation---" + eventBaseAnnotation);
                //如果设置为EventBase
                if (eventBaseAnnotation != null)
                {
                    ...
                    String listenerSetter = eventBaseAnnotation
                            .listenerSetter();
                    Log.d("ggg", "listenerSetter---" + listenerSetter);
                    Class<?> listenerType = eventBaseAnnotation.listenerType();
                    Log.d("ggg", "listenerType---" + listenerType);
                    String methodName = eventBaseAnnotation.methodName();
                    Log.d("ggg", "methodName---" + methodName);
